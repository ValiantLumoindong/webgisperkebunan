            ...
            <div class="block-content block-content-full">
                <form action="{{ route('kebun.prosesupdate', $kebun->id)}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control" required name="id_kecamatan">
                                ...
                                @foreach ($kecamatan as $tampilkecamatan)
                                    <option value="{{ $tampilkecamatan->id }}" {{ $kebun->id_kecamatan == $tampilkecamatan->id ? 'selected' : '' }}>{{ $tampilkecamatan->nama_kecamatan }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <select class="form-control" required name="jenis_kebun">
                                ...
                                @foreach ($jenis_kebun as $tampilkebun)
                                    <option value="{{ $tampilkebun->id }}" {{ $kebun->jenis_kebun == $tampilkebun->id ? 'selected' : '' }}>{{ $tampilkebun->jenis_kebun }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            <input type="color" class="form-control" value="{{ $kebun->warna_zone }}" required name="warna_zone">
                        </div>
                    </div>
                    <br>
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            ...
                            <strong class="text-info">
                                <a href="{{ url ('gambarkebun/') }}/{{ $kebun->gambar }}" target="_blank">Gambar Sekarang: {{ $kebun->gambar }}</a>
                            </strong>
                            @error('gambar')
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            <textarea name="informasi_tambahan" class="form-control">{{ $kebun->informasi_tambahan }}</textarea>
                        </div>
                    </div>
                    <br>
                    <div class="row">
